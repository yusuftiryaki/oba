...
class OBArobot:
        ...
        try:
            ...
            if hasattr(self.main_controller, 'stats'):
                ...
                if runtime > 8 * 3600:  # 8 saat
                    self.logger.warning("Maksimum çalışma süresi aşıldı, şarja dönülüyor")
                    if self.main_controller.state.value != "charging":
                        self.main_controller.state = self.main_controller.RobotState.RETURNING_TO_CHARGE
    ...
    def status(self):
        ...
        if self.main_controller:
            ...
            print(f"Pozisyon: X:{status['position']['x']:.2f}, Y:{status['position']['y']:.2f}")
...
def main():
    ...
    parser.add_argument('--status', '-s', action='store_true',
                       help='Sadece durumu göster')
    parser.add_argument('--test', '-t', action='store_true',
                       help='Test modu')
    parser.add_argument('--web-only', '-w', action='store_true',
                       help='Sadece web arayüzünü başlat')
